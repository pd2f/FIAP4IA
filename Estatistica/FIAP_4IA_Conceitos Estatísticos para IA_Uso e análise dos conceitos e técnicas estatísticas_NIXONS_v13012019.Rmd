...
#Base de vinhos portugueses (brancos e tintos), para estudo de qualidade dos mesmos.
#Amostra aleatória para disparar campanha.
#Variave qualidade (quality 3 a 9). Váriavel target (ou resposta)
...
Vinhos <- read.csv("BaseWine_Red_e_White2018.csv", sep=";")

#Considerando apenas os vinhos brancos
Vinho_branco <- subset(Vinhos, Vinho=="WHITE", select=c(quality,fixedacidity,volatileacidity,citricacid,residualsugar,chlorides,freesulfurdioxide,totalsulfurdioxide,density,pH, sulphates,alcohol))


sort(unique(Vinho_branco$quality))


# factor(Vinho_branco$quality, ordered=TRUE,
#         labels=c("Ruim","Ruim","Regular","Regular","Bom", "Bom", "Ótimo"))
...
#Váriável estava númerica, vamos usar o comando factor para:
#Transformar a variavel em categorica pois estamos utilizamos uma técnica discriminatória
Vinho_branco$quality <- factor(Vinho_branco$quality)

# Frequência absoluta 
table(Vinho_branco$quality)
...
plot(Vinho_branco$fixedacidity, Vinho_branco$quality,ylab="Qualidade",xlab="fixedacidity",col=c('red','darkgreen'))

plot(Vinho_branco$volatileacidity, Vinho_branco$quality,ylab="Qualidade",xlab="volatileacidity",col=c('red','darkgreen'))

plot(Vinho_branco$citricacid, Vinho_branco$quality,ylab="Qualidade",xlab="citricacid",col=c('red','darkgreen'))

#plot(Vinho_branco$residualsugar, Vinho_branco$quality,ylab="Qualidade",xlab="residualsugar",col=c('red','darkgreen'))

plot(Vinho_branco$chlorides, Vinho_branco$quality,ylab="Qualidade",xlab="chlorides",col=c('red','darkgreen'))

#plot(Vinho_branco$freesulfurdioxide, Vinho_branco$quality,ylab="Qualidade",xlab="freesulfurdioxide",col=c('red','darkgreen'))

plot(Vinho_branco$totalsulfurdioxide, Vinho_branco$quality , ylab="Qualidade",xlab="dioxido de enxofre total",col=c('red','darkgreen'))

plot(Vinho_branco$density, Vinho_branco$quality,ylab="Qualidade",xlab="Densidade",col=c('red','darkgreen'))

plot(Vinho_branco$pH, Vinho_branco$quality,ylab="Qualidade",xlab="pH",col=c('red','darkgreen'))

plot(Vinho_branco$sulphates, Vinho_branco$quality,ylab="Qualidade",xlab="sulphates",col=c('red','darkgreen'))

plot(Vinho_branco$alcohol, Vinho_branco$quality,ylab="Qualidade",xlab="alcohol",col=c('red','darkgreen'))

#A variável Vinho será desconsiderada, pois filtramos a base apenas para vinho branco (WHITE)
#plot(Vinho_branco$Vinho, Vinho_branco$quality,ylab="Qualidade",xlab="Vinho",col=c('red','darkgreen'))


par (mfrow=c(1,1))#voltar a tela 1 por 1

#Divisao do banco de dados completo em treinamento e teste
#definir % de casos de treino
prt <- 2/3 #Pegar 2/3 para treinar

# amostra de casos de treino aleatória
set.seed(2019)#semente aleatória
treino <- sample(1:NROW(Vinho_branco), as.integer(prt*NROW(Vinho_branco)))

trainData <- Vinho_branco[treino,]#Pegando parte da amostra
testData  <- Vinho_branco[-treino,]#jogando o resto na base teste
...
# modelo_tree <- rpart (quality ~ fixedacidity + volatileacidity + citricacid  + chlorides + totalsulfurdioxide + density + pH + sulphates + alcohol , data=trainData, cp = 0.006,minsplit = 150,maxdepth=10)#150 observações e 10 níveis. #cp é a sensibilidade


# modelo_tree <- rpart (quality ~ totalsulfurdioxide + density , data=trainData, cp = 0.006,minsplit = 150,maxdepth=10)#150 observações e 10 níveis. #cp é a sensibilidade
